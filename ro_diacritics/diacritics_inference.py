...
MODEL_URL = "https://github.com/AndyTheFactory/andythefactory.github.io/raw/main/diacritice.zip"
...
def restore_diacritics(text, batch_size=128):
    ...
    for idx, p in d.items():
        ...
        if text[idx].lower()==text[idx]:
            if chr =='â' and (idx==0 or idx==len(text)-1 or not text[idx-1].isalpha() or not text[idx+1].isalpha()):
                #  â is not admitted at the beginning or end
                continue
            if chr =='î' and (idx>0 and idx<len(text)-1 and text[idx-1].isalpha() and text[idx+1].isalpha()):
