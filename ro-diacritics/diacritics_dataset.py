...
from .diacritics_utils import correct_diacritics, remove_diacritics, has_interesting_chars, DIACRITICS_CANDIDATES
...
class DiacriticsVocab:
    def __init__(self, distinct_tokens: Counter, max_vocab, pad_token, unk_token, max_char_vocab, overflow_char):
        self.vocab = {
            "itos": {},
            "stoi": {},
        ...
        for word, index in embedding.stoi.items():  # Aggregate embeddings with diacritics
            word = remove_diacritics(word)
            if word in self.vocab["stoi"]:
                idx = self.vocab["stoi"][word]
                self.vocab["vectors"][idx] = self.vocab["vectors"][idx] + embedding.vectors[index]

        for word, index in self.vocab["stoi"].items():
            if fasttext_counts[word] > 1:
                self.vocab["vectors"][index] = self.vocab["vectors"][index] / fasttext_counts[word]
...
class DiacriticsDataset(IterableDataset):
    ...
    def __init__(self, data, character_window, sentence_window, min_line_length=50, max_vocab=25000,
                 max_char_vocab=770, overflow_char=255, diacritics_vocab: DiacriticsVocab = None):
        """
        ...
        self.diacritics_vocab = DiacriticsVocab(distinct_tokens, self.max_vocab,
                                                self.pad_token, self.unk_token,
                                                self.max_char_vocab, self.overflow_char
                                                ) if diacritics_vocab is None else diacritics_vocab
    ...
    def get_label(original_char):
        ...
        label = diacritic_to_label[original_char] if original_char in diacritic_to_label else 0
    ...
    def get_char_input(self, line, line_orig, token_idx):
        ...
        for ix, c in enumerate(word):
            if c in DIACRITICS_CANDIDATES:
                ...
                l = [self.pad_character] * (self.character_window - len(encode_text)) + list(
                    map(self.encode_char, encode_text)) + [
                        self.encode_char(c)]
                encode_text = (word[ix + 1:] + " " + suffix_s).strip()
                encode_text = encode_text[:self.character_window]
                l = l + list(map(self.encode_char, encode_text)) + [self.pad_character] * (
                            self.character_window - len(encode_text))
    ...
    def get_sentence_emb(self, sentence):
        ...
        if len(encoded) < self.sentence_window:
            encoded = encoded + [self.get_word_emb("<pad>")] * (self.sentence_window - len(encoded))
    ...
    def parse_text(self):
        ...
        for line_orig in self.texts:
            line = [remove_diacritics(x).lower() for x in line_orig]  # embeddings based on words without diacritics
            line_indices = np.arange(len(line))
            if len(line_indices) > self.sentence_window:
                windows = np.lib.stride_tricks.sliding_window_view(line_indices, self.sentence_window)
            else:
            ...
            for window in windows:
                ...
                for token_idx in window:
                    if has_interesting_chars(line[token_idx]):
                        char_inputs, labels, _ = self.get_char_input(line, line_orig, token_idx)
                        word_emb = self.get_word_emb(line[token_idx])
                        for ix, char_input in enumerate(char_inputs):
                            yield (torch.tensor(char_input), word_emb, sentence_emb), labels[ix]
    ...
    def gen_batch(self,text, stride=1):
        ...
        for line_span in lines:
            ...
            if len(words) > self.sentence_window:
                windows = np.lib.stride_tricks.sliding_window_view(word_indices, self.sentence_window)[::stride, :]
                # add a reminder window (if it does not fit perfectly)
                if windows.max()<max(word_indices):
                    windows = np.vstack([windows, word_indices[-self.sentence_window:]])
            else:
                windows = [word_indices]

            for window in windows:
                sentence = [line[x[0]:x[1]] for x in words[window[0]:window[-1] + 1]]
                sentence_emb = self.get_sentence_emb(sentence)
                for token_idx in window:
                    word = line[words[token_idx][0]:words[token_idx][1]]
                    if has_interesting_chars(word):
                        char_inputs, _, char_positions = self.get_char_input(line_tokens, line_tokens, token_idx)
                        word_emb = self.get_word_emb(word)
                        for ix, char_input in enumerate(char_inputs):
                            input_tensors.append([torch.tensor(char_input), word_emb, sentence_emb])
                            character_indices.append(line_span[0] + words[token_idx][0] + char_positions[ix])
        ...
        if data and Path(data).exists():
            ...
            if Path(filename).suffix.lower() == '.pkl':  # loading cached pickle
                ...
                with open(filename, "r", encoding="utf-8") as f:
                    texts = [correct_diacritics(line) for line in f if len(line) > min_line_length]

                texts = [x for line in texts for x in sent_tokenize(line, 'english') if len(x) > min_line_length]
                texts = [word_tokenize(line) for line in texts]

                distinct_no_diacritics = Counter([remove_diacritics(x) for line in texts for x in line])
        else:
            texts = [x for  x in sent_tokenize(data, 'english') if len(x) > min_line_length]
            texts = [word_tokenize(line) for line in texts]
