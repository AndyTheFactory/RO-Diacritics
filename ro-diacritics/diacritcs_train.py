...
def train(model, loss_func, train_dataloader:DataLoader, valid_dataloader:DataLoader, epochs=10, checkpoint_file=None):

    optimizer = optim.Adam(model.parameters(), lr=0.001)
    device = next(model.parameters()).device
    print(f"{device} device for training")
    # initialize running values
    history = {"train_loss":[], "valid_loss": [], "acc": [], "f1":[], "epoch":[], "step":[]}
    best_acc = 0
    best_acc_epoch = 0
    nr_non_improving = 0
    patience = 3
    ...
    def evaluate_step(step, epoch, running_loss, max_eval_steps=None):
        ...
        if valid_dataloader is None: return

        print(f"Evaluating (Epoch/Step) {epoch +1} / {step}: ")
        average_train_loss = running_loss / step
        valid_loss, valid_acc, valid_f1 = evaluate(model, valid_dataloader, loss_func, epoch, max_eval_steps)
        ...
        print(f"Epoch [{epoch + 1}/{epochs}] - step {step}, Train Loss: {average_train_loss}, " 
        ...
        if round(best_acc,5) < round(valid_acc,5):
            ...
            if checkpoint_file is not None:
                model.save(checkpoint_file, train_dataloader.dataset.diacritics_vocab,
    ...
    for epoch in range(epochs):
        ...
        for (char_input, word_emb, sentence_emb), labels in train_dataloader:
            ...
            logits = model(char_input.to(device), word_emb.to(device), sentence_emb.to(device))
            ...
            if step % 1000 == 0:
                ...
                epoch_f1_metrics = f1_score(np.array(epoch_true_labels).reshape(-1),
                                      np.array(epoch_predictions).reshape(-1), average='weighted')
                average_train_loss = running_loss / step
                print(f"Epoch {epoch+1} / Step: {step}, Loss: {loss.item()}, Avg Loss: {average_train_loss} , Train ACC: {epoch_accuracy}, Train F1: {epoch_f1_metrics}")

            if step % 10000 == 0:
                evaluate_step(step, epoch, running_loss, 300_000 // valid_dataloader.batch_size)
                model.train()
...
def evaluate(model, dataloader:DataLoader, loss_func, epoch=None, max_eval_steps=None):
    ...
    device = next(model.parameters()).device
    with torch.no_grad():
        for (char_input, word_emb, sentence_emb), labels in tqdm(dataloader, total=max_eval_steps):
            labels = labels.to(device)
            logits = model(char_input.to(device), word_emb.to(device), sentence_emb.to(device))
    ...
    if (epoch):
        print(f"Evaluation at Epoch: {epoch +1 }, Acc={eval_acc}, F1={f1_metrics}, Loss={eval_loss}")
    else:
